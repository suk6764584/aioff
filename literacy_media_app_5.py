...
from literacy_cases_4 import CASE_LIBRARY

# literacy_media_app_3의 채팅 내 실제 사례 선택/대화/AI OFF 흐름은 유지하고,
# 임의 요약 대신 원문에서 직접 짧게 발췌한 문장을 사례 카드에 표시합니다.
...
def _public_case_with_excerpt(case):
    data = {
        key: case[key]
        for key in (
    ...
    data["source_excerpt"] = case.get("source_excerpt", "")
    return data


flow._public_case = _public_case_with_excerpt


def _render_index_v5():
    html = flow._render_index()

    # 판단 전에 모델이 만든 설명이 아니라 실제 원문 일부를 먼저 읽게 합니다.
    old = '<div class="chat-case-claim"><b>당시 퍼진 주장·상황</b>${esc(c.claim)}</div>'
    new = (
        '<div class="chat-case-summary"><b>원문 발췌</b><span class="source-quote">“${esc(c.source_excerpt||c.claim)}”</span></div>'
    ...
    css_new = (
        ...
        '.chat-case-summary .source-quote{display:block;font-style:normal;color:var(--body)}'
    ...
    html = html.replace(
        ...
        '사례를 선택하면 사진·기사·문서와 원문 발췌, 판단할 주장을 보여주고 AI가 바로 첫 질문을 합니다.',
    )
    return html


# 이전 버전의 루트 화면만 제거하고 원문 발췌 화면으로 다시 등록합니다.
flow.base._remove_route("/", "GET")


@app.get("/", response_class=HTMLResponse)
def inline_case_index_v5():
    return HTMLResponse(_render_index_v5())
