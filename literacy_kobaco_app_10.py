...
# v10에서는 학생 화면에 확인된 사례 정보와 실제 데이터만 표시합니다.
app = previous.app
base = previous.base
flow = previous.flow
v5 = previous.v5
    ...
    for idx, row in enumerate(rows, 1):
        ...
        cases.append(
            src._common_case(
                ...
                label="청소년 OTT 실제 통계",
                title=f"{year} · {group}",
                claim=f"{year}년 {group}의 플랫폼별 이용 비율을 확인하는 KOBACO 통계입니다.",
                ...
                resolution=(
                    ...
                    "선호도나 다른 연령집단의 결과는 이 표만으로 확인할 수 없습니다."
                ),
                opening_questions=[
                    f"{year}년 {group} 자료에서 가장 먼저 눈에 들어오는 수치를 하나 골라, 그 수치가 무엇을 뜻하는지 적어보세요.",
                    "가장 높은 이용 비율을 보고 '가장 좋아하는 서비스'라고 말할 수 있는지 표의 항목을 근거로 판단해보세요.",
                    "이 결과를 다른 연령에게 그대로 적용할 수 있는지 조사 대상을 보고 설명해보세요.",
                ],
...
RELATED_CONTEXT = {
    "무주반딧불축제": {
        ...
        "summary": (
            "제30회 무주반딧불축제는 2026년 9월 4일부터 12일까지 무주 일원에서 열리는 "
            "생태환경축제로, 반딧불이 신비탐사·생태체험과 30주년 프로그램 등이 운영됩니다."
        ),
    },
}
...
def _attach_aisac_context(case: dict[str, Any]) -> None:
    ...
    matched = next((value for key, value in RELATED_CONTEXT.items() if key in title), None)
    if matched:
        case["context_url"] = matched["url"]
        case["context_label"] = matched["label"]
        case["context_summary"] = matched["summary"]
    else:
        # 확인된 외부 맥락이 없으면 설명문을 만들어내지 않습니다.
        case["context_url"] = ""
        case["context_label"] = ""
    ...
    if matched:
        questions = [
            f"'{title}'의 관련 자료와 원본 광고를 본 뒤, 이 광고가 무엇을 홍보하는지 한 문장으로 적어보세요. 이어서 AiSAC 키워드 중 실제 내용과 맞는 단어 하나를 골라보세요.",
            f"'{title}'의 원본에서 확인한 내용과 AiSAC 키워드가 서로 잘 맞는 부분 하나를 찾아보세요.",
            "관련 자료와 AI 인식 결과 중 각각 어디에서 확인한 정보인지 구분해서 한 가지씩 적어보세요.",
        ]
    else:
        questions = [
            f"'{title}' 원본 광고를 열어 무엇을 홍보하는 광고인지 한 문장으로 적고, AiSAC 키워드 중 실제 영상에서 확인되는 단어 하나를 골라보세요.",
            f"'{title}' 원본에서 확인한 장면이나 문구 하나와 AiSAC 키워드 하나를 짝지어보세요.",
            "원본 광고에서 직접 확인한 사실 하나와 AI가 인식한 항목 하나를 구분해서 적어보세요.",
        ]
...
'''
        ...
        stage = f'''
학생은 원본 광고와 확인된 관련 자료가 있으면 그것을 먼저 보고 AiSAC 인식값과 비교한다.
현재 학생 답변 횟수: {turn}
- 학생이 실제 원본/관련 자료에서 확인한 내용과 AI가 찾은 키워드를 구분한다.
- 화면에 없는 광고 내용이나 의도를 만들어내지 않는다.
- 외부 맥락 요약이 비어 있으면 그 사례의 배경을 아는 것처럼 설명하지 않는다.
- 학생이 원본을 보지 않았다면 원본 광고 버튼을 이용하도록 짧게 안내한다.
'''
    elif kind == "ott":
        stage = f'''
...
'''
    ...
    context = str(case.get("context_summary") or "").strip()
    context_block = context if context else "(확인된 외부 맥락 요약 없음)"
...
7. 확인된 외부 맥락이 비어 있으면 새로운 배경 설명을 생성하지 않는다.
8. 같은 질문을 표현만 바꿔 반복하지 않는다.
9. AI OFF 최종 문제는 지금 미리 출제하지 않는다.
...
def _render_index_kobaco_v10():
    ...
    replacements = {
        ...
        "공식 교육·조사자료를 바탕으로 정리한 뉴스·딥페이크·AI 답변 검증 기준을 사례로 연습합니다. 학습을 마치면 AI OFF가 방금 대화를 분석해, 같은 판단 기준을 혼자 적용해보는 문제 3개를 만듭니다.": "실제 KOBACO 광고·공익광고·청소년 미디어 이용 자료를 보고, 자료에 나온 사실과 내 해석을 구분해봅니다.",
        ...
        "사례를 선택하면 사진·기사·문서와 DB 조회값, 판단할 주장을 보여주고 AI가 바로 첫 질문을 합니다.": "사례를 선택하면 실제 자료와 확인된 관련 정보를 먼저 보여줍니다.",
...
.fact-grid{display:grid;grid-template-columns:1.4fr 1fr 1fr;gap:8px;padding:12px 16px;background:#f7f4ef;border-bottom:1px solid #e2dbd1}.fact-grid div{background:#fff;border:1px solid #ddd5ca;border-radius:8px;padding:10px}.fact-grid small{display:block;font-size:9px;color:#7b7168;margin-bottom:4px}.fact-grid b{font-size:11px;line-height:1.45;color:#2c2824;word-break:break-word}
.verified-context{padding:14px 16px;background:#fff;border-bottom:1px solid #e2dbd1}.verified-context small{display:block;font-size:9px;font-weight:900;color:#766b61;margin-bottom:6px}.verified-context p{font-size:11px;line-height:1.65;color:#514a44;margin:0}
.aisac-result{padding:14px 16px;background:#fff}.aisac-result small{display:block;font-size:9px;font-weight:900;color:#756a60;margin-bottom:6px}.aisac-result strong{display:block;font-size:13px;line-height:1.6;color:#27231f}.aisac-result p{font-size:10px;line-height:1.55;color:#6a6159;margin:6px 0 0}
...
@media(max-width:650px){.fact-grid,.ott-context,.kobaco-readable-metrics{grid-template-columns:1fr}.ott-bar{grid-template-columns:85px 1fr 44px}}
'''
...
function fixedDataMedia(c,kind,title,intro){{return `<div class="chat-case-media"><div class="kobaco-data-card"><div class="context-card"><b>${{esc(kind)}} · ${{esc(title)}}</b>${{intro?`<p>${{esc(intro)}}</p>`:''}}</div><div class="kobaco-data-body"><div class="kobaco-data-table">${{fixedRawRows(c)}}</div></div></div></div>`;}}
...
function aisacLearningCard(c){{
  const r=fixedRows(c);
  const related=c.context_url||relatedSearchUrl(c);
  const relatedLabel=c.context_url?(c.context_label||'관련 자료 보기'):'관련 정보 검색';
  ...
  return `<div class="chat-case-media"><div class="kobaco-data-card">
    <div class="context-card"><b>사례 기본정보</b><div class="context-actions"><a href="${{aisacSearchUrl(c)}}" target="_blank" rel="noopener">AiSAC 원본 광고 찾기 ↗</a><a class="alt" href="${{esc(related)}}" target="_blank" rel="noopener">${{esc(relatedLabel)}} ↗</a></div></div>
    <div class="fact-grid"><div><small>광고명</small><b>${{esc(c.title||'-')}}</b></div><div><small>광고주</small><b>${{esc(r['광고주']||'-')}}</b></div><div><small>업종</small><b>${{esc(r['업종']||'-')}}</b></div></div>
    ${{verified}}
...
function ottLearningCard(c){{const r=fixedRows(c);const group=r['연도·집단']||c.title||'-';const sample=r['사례수']||'-';const nonuse=r['OTT 비이용']||'-';const pairs=ottPairs(r['이용비율 상위']);const bars=pairs.map(p=>`<div class="ott-bar"><span>${{esc(p.name)}}</span><span class="ott-bar-track"><i style="width:${{Math.max(0,Math.min(100,p.value))}}%"></i></span><span class="ott-bar-value">${{p.value.toFixed(1)}}%</span></div>`).join('');return `<div class="chat-case-media"><div class="kobaco-data-card"><div class="ott-summary"><b>KOBACO 청소년 OTT 이용 통계</b><br>${{esc(group)}} 응답자의 서비스별 이용 비율을 보여줍니다.</div><div class="ott-context"><div><small>조사 대상</small><b>${{esc(group)}}</b></div><div><small>사례수</small><b>${{esc(sample)}}</b></div><div><small>OTT 비이용</small><b>${{esc(nonuse)}}</b></div></div><div class="ott-bars"><b>서비스별 이용 비율 상위</b>${{bars}}</div><div class="ott-summary"><b>표의 항목:</b> 서비스별 이용 비율입니다. 선호도·만족도 항목은 이 자료에 포함되어 있지 않습니다.</div><details class="kobaco-evidence"><summary>실제 조사 데이터 항목 보기</summary><div class="kobaco-data-body"><div class="kobaco-data-table">${{fixedRawRows(c)}}</div></div></details></div></div>`;}}
function publicAdCard(c){{const r=fixedRows(c);const metric=(label,value,meaning)=>`<div class="kobaco-readable-metric"><small>${{esc(label)}}</small><strong>${{esc(value||'-')}}</strong><p>${{esc(meaning)}}</p></div>`;const archive=c.archive_url?`<a href="${{esc(c.archive_url)}}" target="_blank" rel="noopener">공식 상세페이지 ↗</a>`:'';const video=c.video_url?`<a class="alt" href="${{esc(c.video_url)}}" target="_blank" rel="noopener">광고 영상 보기 ↗</a>`:'';return `<div class="chat-case-media"><div class="kobaco-data-card"><div class="kobaco-thumb-stage"><img src="/api/kobaco-media-thumb/${{encodeURIComponent(c.id)}}" alt="공익광고 썸네일" loading="eager"></div><div class="context-card"><b>${{esc(c.archive_title||c.title||'KOBACO 공익광고')}}</b><div class="context-actions">${{archive}}${{video}}</div></div><div class="kobaco-after-answer" data-kobaco-after-answer><div class="kobaco-readable-metrics">${{metric('광고의 신뢰성을 평가한 항목',r['신뢰성'],'행동 변화 비율을 뜻하지 않습니다.')}}${{metric('광고를 접한 경로 중 가장 높은 항목',r['주요 인지경로'],'광고의 좋고 나쁨을 평가한 점수가 아닙니다.')}}${{metric('가장 강한 인상을 준 요소',r['임팩트 1위'],'광고 전체 효과를 하나의 숫자로 나타낸 값이 아닙니다.')}}</div><details class="kobaco-evidence" open><summary>실제 조사 원자료 항목 보기</summary><div class="kobaco-data-body"><div class="kobaco-data-table">${{fixedRawRows(c)}}</div></div></details></div></div></div>`;}}
// 세 데이터 유형을 여기서 직접 렌더링해 재귀 호출을 완전히 끊습니다.
function caseMedia(c){{const id=String(c?.id||'');if(id.startsWith('kobaco_publicad_'))return publicAdCard(c);if(id.startsWith('kobaco_aisac_'))return aisacLearningCard(c);if(id.startsWith('kobaco_ott_'))return ottLearningCard(c);return fixedDataMedia(c,'KOBACO 실제 자료',c.title||'자료','');}}
